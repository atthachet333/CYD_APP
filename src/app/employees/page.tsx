...
interface PageProps {
  searchParams: Promise<{ viewId?: string; docId?: string; moveId?: string; deleteId?: string }>;
}
...
async function deleteEmployeeAction(formData: FormData) {
  ...
  if (id) await prisma.employee_document_profiles.delete({ where: { id: Number(id) } });
  redirect("/employees");
}
...
export default async function EmployeesPage({ searchParams }: PageProps) {
  ...
  // 1. ดึงข้อมูล User จากฐานข้อมูลให้ชัวร์ (ใช้เทคนิค NO_MATCH กันบั๊กค่าว่าง)
  ...
  // 2. ตรวจสอบสิทธิ์และดึง ID บริษัท
  ...
  if (role === "CUSTOMER") redirect("/company-dashboard");
  const canViewDocs = role !== "CUSTOMER";

  // 🔒 3. สร้างระบบ Filter: ถ้าเป็น COMPANY_USER ให้ดึงแค่บริษัทตัวเองเท่านั้น
  const employeeFilter: any = {};
  if (role === "COMPANY_USER" && userCompanyId) {
    employeeFilter.company_id = Number(userCompanyId);
  }

  // 4. สั่งดึงข้อมูลโดยแนบ Filter ไปด้วย
            ...
            <div className="p-6 space-y-4">
              <div className="p-4 border border-gray-200 rounded-2xl flex justify-between items-center bg-white shadow-sm">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">ไฟล์แนบรวม</p>
                </div>
