...
export default async function NinetyDayRenewalPage() {
  // 1. ดึงข้อมูลพนักงานทั้งหมดเพื่อนำมาคำนวณสถิติ 4 กล่อง
  ...
  // วันนี้ (เที่ยงคืน) และ อีก 7 วันข้างหน้า
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const in7Days = new Date(today);
  in7Days.setDate(today.getDate() + 7);

  // อาร์เรย์สำหรับเก็บเฉพาะคนที่มีข้อมูลเพื่อแสดงในตาราง
  const tableData: typeof allProfiles = [];

  // 2. วนลูปคำนวณสถิติ
  allProfiles.forEach(p => {
    // *** สำหรับหน้าอื่น ให้เปลี่ยนตรง p.ninety_day_report_date เป็นฟิลด์ของหน้านั้นๆ ***
    if (!p.ninety_day_report_date) {
      noData++;
    } else {
      tableData.push(p);
      const targetDate = new Date(p.ninety_day_report_date);
  ...
  // เรียงลำดับในตารางจากวันที่ใกล้ถึงก่อน
  tableData.sort((a, b) => new Date(a.ninety_day_report_date!).getTime() - new Date(b.ninety_day_report_date!).getTime());
  ...
  const getStatus = (date: Date) => {
    ...
    if (diffDays <= 30) return { label: "ใกล้ถึงกำหนด", color: "bg-yellow-100 text-yellow-700", days: diffDays };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">แจ้งเตือนต่อเอกสาร 90 วัน</h1>
        <p className="text-sm text-gray-500 mt-1">รายการพนักงานที่ต้องรายงานตัว 90 วัน</p>
      </div>

      {/* 3. ส่วนการ์ดสถิติ 4 ช่อง (ตามภาพที่ส่งมา) */}
      ...
      {/* 4. ตารางแสดงผล (แสดงเฉพาะคนที่มีข้อมูล) */}
          ...
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-white text-gray-500 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-center">วันที่กำหนดรายงานตัว</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {tableData.length === 0 ? (
                ...
                tableData.map((p) => {
                  const status = getStatus(p.ninety_day_report_date!);
                  return (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-center font-mono">
                        {p.ninety_day_report_date?.toLocaleDateString("th-TH", { year: "numeric", month: "short", day: "numeric" })}
                      </td>
