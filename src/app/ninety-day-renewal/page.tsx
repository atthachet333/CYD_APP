...
import NinetyDayRenewalClient from "./NinetyDayRenewalClient";
...
export default async function NinetyDayRenewalPage() {
  ...
  const profiles = await prisma.employee_document_profiles.findMany({
    orderBy: [{ ninety_day_report_date: "asc" }, { created_at: "desc" }],
    select: {
      id: true,
      emp_code: true,
      first_name: true,
      ...
      first_name_en: true,
      last_name_en: true,
      ninety_day_number: true,
      ninety_day_report_date: true,
      report_90_days_date: true,
      ninety_day_file: true,
    },
  });
  ...
  for (const profile of profiles) {
    const date = profile.ninety_day_report_date || profile.report_90_days_date || null;
  ...
  const employees = profiles.map((profile) => ({
    ...
    employeeName:
      ...
      profile.emp_code ||
      `employee-${profile.id}`,
    ninety_day_number: profile.ninety_day_number,
    ninety_day_report_date: isoDate(profile.ninety_day_report_date),
    report_90_days_date: isoDate(profile.report_90_days_date),
    ninety_day_file: profile.ninety_day_file,
  }));

  return (
    <div className="mx-auto min-h-screen w-full max-w-screen-2xl bg-gray-50 p-3 sm:p-4 md:p-6 lg:p-8">
      <div className="mb-6">
        <h1 className="break-words text-xl font-bold text-gray-900 sm:text-2xl lg:text-3xl">แจ้งเตือนต่อเอกสาร 90 วัน</h1>
        <p className="text-sm text-gray-500 mt-1">รายการพนักงานที่ต้องรายงานตัว 90 วัน และบันทึกการต่อเอกสารจากฐานข้อมูลจริง</p>
      </div>
      ...
      <NinetyDayRenewalClient employees={employees} />
    </div>
  );
}
