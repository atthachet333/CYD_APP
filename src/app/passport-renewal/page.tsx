...
export default async function PassportRenewalPage() {
  ...
  allProfiles.forEach(p => {
    if (!p.passport_expiry_date) {
      ...
      tableData.push(p);
      const targetDate = new Date(p.passport_expiry_date);
  ...
  tableData.sort((a, b) => new Date(a.passport_expiry_date!).getTime() - new Date(b.passport_expiry_date!).getTime());
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">รายการหนังสือเดินทาง (Passport) ที่ใกล้หมดอายุ</p>
      </div>
          ...
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-white text-gray-500 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-center">วันที่พาสปอร์ตหมดอายุ</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {tableData.length === 0 ? (
                ...
                tableData.map((p) => {
                  const status = getStatus(p.passport_expiry_date!);
                  return (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-center font-mono">
                        {p.passport_expiry_date?.toLocaleDateString("th-TH", { year: "numeric", month: "short", day: "numeric" })}
                      </td>
