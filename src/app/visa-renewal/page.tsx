...
export default async function VisaRenewalPage() {
  ...
  allProfiles.forEach(p => {
    if (!p.visa_expiry_date) {
      noData++;
    } else {
      tableData.push(p);
      const targetDate = new Date(p.visa_expiry_date);
  ...
  tableData.sort((a, b) => new Date(a.visa_expiry_date!).getTime() - new Date(b.visa_expiry_date!).getTime());
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">รายการวีซ่า (Visa) ที่ใกล้หมดอายุ</p>
      </div>
          ...
          <table className="w-full text-left text-sm text-gray-600">
            <thead className="bg-white text-gray-500 border-b border-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-center">วันที่วีซ่าหมดอายุ</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {tableData.length === 0 ? (
                ...
                tableData.map((p) => {
                  const status = getStatus(p.visa_expiry_date!);
                  return (
                    <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-center font-mono">
                        {p.visa_expiry_date?.toLocaleDateString("th-TH", { year: "numeric", month: "short", day: "numeric" })}
                      </td>
