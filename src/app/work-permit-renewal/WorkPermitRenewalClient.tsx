...
export default function WorkPermitRenewalClient({ employees }: { employees: EmployeeRow[] }) {
  ...
  const filteredEmployees = useMemo(() => {
    return employees.filter((emp) => {
      ...
      const num = workPermitNumber(emp).toLowerCase();
      const matchSearch =
        !keyword ||
      ...
      const statusObj = statusFor(emp.work_permit_expiry_date);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
        <div className="p-4 border-b border-gray-100 bg-gray-50/50">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-[1fr_1fr_auto] items-end">
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-2">ค้นหาพนักงาน (ชื่อ, รหัส, เลขใบอนุญาต)</label>
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="พิมพ์ค้นหา..." className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-2">กรองตามสถานะ</label>
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
            ...
            <div className="flex">
              <button type="button" onClick={clearFilters} className="px-6 py-2.5 bg-gray-200 text-gray-700 font-bold rounded-xl text-sm hover:bg-gray-300 transition text-center flex items-center justify-center w-full sm:w-auto">ล้างตัวกรอง</button>
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap min-w-[1080px]">
            <thead className="bg-slate-50/80 text-gray-500 border-b border-gray-100 uppercase tracking-wider text-[11px] md:text-xs">
              <tr>
                ...
                <th className="p-4 font-bold">เลขใบอนุญาต</th>
              ...
              {paginatedEmployees.map((employee) => {
                ...
                return (
                  <tr key={employee.id} className="hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="p-4 text-center font-medium text-gray-600">{formatThaiDate(employee.work_permit_expiry_date)}</td>
                    <td className="p-4 text-center font-bold text-gray-800">{status.daysText}</td>
                    <td className="p-4 text-center">
                      <span className={`inline-flex px-3 py-1 rounded-full text-xs font-bold ${status.color}`}>{status.label}</span>
                    </td>
                    <td className="p-4 text-center">
                      {employee.has_work_permit_file ? (
                        <SecureDocumentButton employeeId={employee.id} documentType="work_permit" className="inline-flex items-center justify-center rounded-lg border border-blue-200 bg-white px-3 py-1.5 text-[11px] font-bold text-blue-600 shadow-sm transition-all hover:bg-blue-600 hover:text-white">เปิดดูไฟล์</SecureDocumentButton>
                      ) : (
                    ...
                    <td className="p-4 text-center pr-6">
                      <button type="button" onClick={() => { setError(""); setSuccess(""); setActiveEmployee(employee); }} className="inline-flex min-w-[140px] items-center justify-center rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white shadow-md shadow-blue-500/20 transition hover:bg-blue-700">ต่อใบอนุญาต</button>
                    </td>
                  </tr>
                );
              })}
        ...
        {totalPages > 1 && (
          <div className="p-4 border-t border-gray-100 bg-gray-50 flex flex-col sm:flex-row items-center justify-between gap-4">
            <span className="text-sm font-medium text-gray-500">แสดง {(currentPage - 1) * ITEMS_PER_PAGE + 1} ถึง {Math.min(currentPage * ITEMS_PER_PAGE, filteredEmployees.length)} จากทั้งหมด {filteredEmployees.length} รายการ</span>
            <div className="flex items-center gap-1.5">
              <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-1.5 rounded-lg border border-gray-200 bg-white text-sm font-bold text-gray-600 hover:bg-gray-50 disabled:opacity-50 shadow-sm transition-colors">&laquo; ก่อนหน้า</button>
              <div className="flex items-center gap-1 hidden sm:flex">
                {Array.from({ length: totalPages }).map((_, idx) => {
                  const pageNum = idx + 1;
                  if (pageNum === 1 || pageNum === totalPages || (pageNum >= currentPage - 1 && pageNum <= currentPage + 1)) {
                    return <button key={pageNum} onClick={() => setCurrentPage(pageNum)} className={`px-3 py-1.5 rounded-lg border text-sm font-bold ${currentPage === pageNum ? 'bg-blue-600 text-white' : 'bg-white text-gray-600'}`}>{pageNum}</button>;
                  }
                  return null;
                })}
              </div>
              <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-3 py-1.5 rounded-lg border border-gray-200 bg-white text-sm font-bold text-gray-600 hover:bg-gray-50 disabled:opacity-50 shadow-sm transition-colors">ถัดไป &raquo;</button>
            </div>
          </div>
        )}
      </div>

      {/* ✅ ปรับปรุงหน้าต่าง Popup Modal ของ Work Permit ใหม่ตามดีไซน์ Visa */}
      {activeEmployee && (
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-blue-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-extrabold text-[#111c44]">บันทึกการต่อใบอนุญาตทำงาน</h2>
                  <p className="text-[11px] font-bold text-blue-600 uppercase tracking-wide">Work Permit Renewal</p>
                </div>
              </div>
              <button 
                type="button"
            ...
            >
              ...
              <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-gray-50/80 p-5 rounded-2xl border border-gray-100">
                  ...
                  <div>
                    <p className="text-[11px] font-bold text-gray-500 mb-1">ข้อมูลใบอนุญาตเดิม</p>
                    <p className="font-bold text-sm text-gray-800">
                      เลขเดิม: <span className="font-medium text-gray-600">{workPermitNumber(activeEmployee) || "-"}</span>
                    </p>
                    <p className="font-bold text-sm text-gray-800 mt-0.5">
                      วันเดิม: <span className="font-medium text-red-500">{formatThaiDate(activeEmployee.work_permit_expiry_date)}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-2">เลขใบอนุญาตใหม่ <span className="text-red-500">*</span></label>
                    <input 
                      name="documentNumber" 
                      defaultValue={workPermitNumber(activeEmployee)} 
                      required
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      name="expiryDate" 
                      required 
                      defaultValue={toInputDate(activeEmployee.work_permit_expiry_date)} 
                      className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm" 
                    />
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-2">ไฟล์ Work Permit ใหม่ (ถ้ามี)</label>
                    <input 
                      type="file" 
                    ...
                    {activeEmployee.has_work_permit_file && (
                      <div className="mt-3 flex items-center gap-2">
                        ...
                        <SecureDocumentButton employeeId={activeEmployee.id} documentType="work_permit" className="px-3 py-1 rounded-lg bg-gray-100 text-gray-700 text-[11px] font-bold hover:bg-gray-200 transition-colors">
                          เปิดดูไฟล์เดิม
