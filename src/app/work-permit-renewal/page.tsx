...
import WorkPermitRenewalClient from "./WorkPermitRenewalClient";
...
export default async function WorkPermitRenewalPage() {
  ...
  const profiles = await prisma.employee_document_profiles.findMany({
    orderBy: [{ work_permit_expiry_date: "asc" }, { created_at: "desc" }],
    select: {
      id: true,
      emp_code: true,
      first_name: true,
      ...
      work_permit_no: true,
      work_permit_expiry_date: true,
      work_permit_file: true,
    },
  });
  ...
  for (const profile of profiles) {
    const days = daysRemaining(profile.work_permit_expiry_date);
  ...
  const employees = profiles.map((profile) => ({
    ...
    employeeName:
      ...
      profile.emp_code ||
      `employee-${profile.id}`,
    work_permit_number: cleanText(profile.work_permit_number),
    work_permit_no: cleanText(profile.work_permit_no),
    work_permit_expiry_date: isoDate(profile.work_permit_expiry_date),
    has_work_permit_file: typedDocumentFileExists(profile, profile.id, "work_permit", "work_permit_file"),
  }));

  return (
    <div className="mx-auto min-h-screen w-full max-w-screen-2xl bg-gray-50 p-3 sm:p-4 md:p-6 lg:p-8">
      <div className="mb-6">
        <h1 className="break-words text-xl font-bold text-gray-900 sm:text-2xl lg:text-3xl">แจ้งเตือนต่อใบอนุญาตทำงาน</h1>
        <p className="text-sm text-gray-500 mt-1">รายการใบอนุญาตทำงานที่ใกล้หมดอายุ และบันทึกการต่อเอกสารจากฐานข้อมูลจริง</p>
      </div>
      ...
      <WorkPermitRenewalClient employees={employees} />
    </div>
  );
}
